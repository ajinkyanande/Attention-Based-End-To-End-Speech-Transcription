...
def experiments(train_dataloader, val_loader, model, criterion, optimizer, lr_scheduler, scaler):
    ...
    for epoch in range(0, CONFIG["epochs"]):
        ...
        (running_loss, running_perplexity, attention_plot) = train(
            train_dataloader, model, criterion, optimizer, scaler, tf_rate
        )
...
def testing(test_dataloader, model):
    ...
    model.eval()
    batch_bar = tqdm(
        total=len(test_dataloader), dynamic_ncols=True, leave=False, position=0, desc="test"
    )
...
def main():
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), lr=CONFIG["cosann_start_lr"], amsgrad=True, weight_decay=5e-6
    )
    criterion = torch.nn.CrossEntropyLoss(reduction="none")

    scaler = torch.cuda.amp.GradScaler()
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=CONFIG["epochs"], eta_min=CONFIG["cosann_min_lr"]
    )
