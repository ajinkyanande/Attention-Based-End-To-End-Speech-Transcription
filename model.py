...
class LockedDropout(torch.nn.Module):
    ...
    def forward(self, inp_pack):
        ...
        mask = inp_pad.new_empty(
            inp_pad.shape[0], 1, inp_pad.shape[2], requires_grad=False
        ).bernoulli_(1 - self.p)
        mask = mask.div_(1 - self.p)
        mask = mask.expand_as(inp_pad)

        out_pad = inp_pad * mask
        out_pack = torch.nn.utils.rnn.pack_padded_sequence(
            out_pad, inp_pad_lens, batch_first=True, enforce_sorted=False
        )
...
class pBLSTM(torch.nn.Module):
    ...
    def forward(self, inp_pack):
        ...
        inp_pack_trunc = torch.nn.utils.rnn.pack_padded_sequence(
            inp_pad_trunc, inp_pad_trunc_lens, batch_first=True, enforce_sorted=False
        )
...
class Listener(torch.nn.Module):
    ...
    def __init__(self, input_size, encoder_hidden_size):
        ...
        self.pBLSTM1 = pBLSTM(
            input_size=encoder_hidden_size * (2 * 2), hidden_size=encoder_hidden_size
        )
    ...
    def forward(self, inp_pad, inp_pad_lens):
        ...
        inp_pack = torch.nn.utils.rnn.pack_padded_sequence(
            inp_pad, inp_pad_lens, batch_first=True, enforce_sorted=False
        )
        ...
        pblstm_pad, pblstm_pad_lens = torch.nn.utils.rnn.pad_packed_sequence(
            pblstm3_lstm_pack, batch_first=True
        )
...
class Attention(torch.nn.Module):
    ...
    def forward(self, decoder_output_embedding):
        ...
        raw_weights = torch.divide(
            torch.bmm(self.key, torch.unsqueeze(self.query, dim=2)), np.sqrt(query_len)
        ).squeeze(2)
...
class Speller(torch.nn.Module):
    ...
    def __init__(
        self, embed_size, decoder_hidden_size, decoder_output_size, vocab_size, attention_module=None
    ):
    ...
    def forward(self, encoder_outputs, encoder_lens, y=None, tf_rate=1):
        ...
        char = torch.full((batch_size,), fill_value=SOS_TOKEN, dtype=torch.long).to(
...
class LAS(torch.nn.Module):
    ...
    def __init__(
        ...
        self.decoder = Speller(
            embed_size, decoder_hidden_size, decoder_output_size, vocab_size, attention_module
        )
